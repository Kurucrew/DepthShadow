...
HRESULT	KViewRT::SetRenderTargetView(ID3D11Texture2D* pBackBuffer)
{
    HRESULT hr = S_OK;
...
bool KViewRT::Create(UINT Width, UINT Height)
{
    ...
    CreateProjMatrix(1.0f, 10000.0f, XM_PI * 0.25f, (float)Width / (float)Height);
...
bool KViewRT::Begin(ID3D11DeviceContext* pContext)
{
    ...
    pContext->OMSetRenderTargets(1, &this->m_pRenderTargetView,
        this->m_dxDs.m_pDepthStencilView);
...
KMatrix KViewRT::CreateProjMatrix(float fNear, float fFar, float fFov, float fAspect)
{
    D3DXMatrixPerspectiveFovLH(&m_matProj, fFov, fAspect, fNear, fFar);
    return m_matProj;
}
HRESULT	KViewRT::SetViewPort(UINT Width, UINT Height)
{
    HRESULT hr = S_OK;
...
void KViewRT::Save(ID3D11DeviceContext* pContext, std::wstring saveFileName)
{
    ...
    HRESULT hr = DirectX::SaveDDSTextureToFile(pContext, pSource, saveFileName.c_str());
}