...
#define KGAME_START int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow ){ Sample gSample;
#define KGAME_WIN(x) if( !gSample.InitWindows( hInstance, nCmdShow,  L#x ) )	{	return 0;	}	ShowCursor ( TRUE );gSample.Run();	return 1; }
#define KGAME_RUN(x) KGAME_START; KGAME_WIN(x);
