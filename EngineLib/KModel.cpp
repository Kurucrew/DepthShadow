...
HRESULT KModel::CreateVertexLayout()
{
    ...
    {
        {"POSITION",0, DXGI_FORMAT_R32G32B32_FLOAT,     0, 0,  D3D11_INPUT_PER_VERTEX_DATA, 0}, //RGB
        {"NORMAL",  0, DXGI_FORMAT_R32G32B32_FLOAT,     0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0}, //RGB
        {"COLOR",   0, DXGI_FORMAT_R32G32B32A32_FLOAT,  0, 24, D3D11_INPUT_PER_VERTEX_DATA, 0}, //RGBA 4°¡Áö
        {"TEXTURE", 0, DXGI_FORMAT_R32G32_FLOAT,        0, 40, D3D11_INPUT_PER_VERTEX_DATA, 0}, //RG
    };
...
ID3DBlob* KModel::LoadShaderBlob(std::wstring vs, std::string function, std::string version)
{
    ...
    hr = D3DCompileFromFile(vs.c_str(), nullptr, nullptr, function.c_str(), version.c_str(),
        0, 0, &ret, &error);
    if (FAILED(hr))
    {
...
HRESULT KModel::LoadShader(std::wstring vs, std::wstring ps)
{
    HRESULT hr = S_OK;
    ID3DBlob* error = nullptr;
    m_pVSBlob = LoadShaderBlob(vs, "VS", "vs_5_0");
    if (m_pVSBlob == nullptr) { return hr; }

    hr = g_pd3dDevice->CreateVertexShader(m_pVSBlob->GetBufferPointer(), m_pVSBlob->GetBufferSize(),
        NULL, &m_pVS);
    if (FAILED(hr)) { return hr; }

    ID3DBlob* PSBlob = nullptr;
    PSBlob = LoadShaderBlob(ps, "PS", "ps_5_0");
    if (PSBlob == nullptr) { return hr; }

    hr = g_pd3dDevice->CreatePixelShader(PSBlob->GetBufferPointer(), PSBlob->GetBufferSize(),
        NULL, &m_pPS);
    if (FAILED(hr)) { return hr; }
    m_pMainPS = m_pPS;
    PSBlob->Release();
...
bool KModel::Create(std::wstring vsFile, std::wstring psFile, std::wstring szTextureName)
{
    if (CreateVertexData())
    {
        CreateVertexBuffer();
